\chapter{Estimation et échantillonnage}
...
\begin{definition}[Echantillon et réalisation]
    Soit $X$ une variable aléatoire sur un espace probabilisé $(\Omega, \myP)$. 
    Un échantillon de $X$ de taille $n \in \N$ est une $n$-uplet de variables aléatoires 
    $(X_1, \dots, X_n)$ iid de même loi que $X$. 
...
\begin{definition}[Fonction de vraissemblance]
    ...
    La fonction de vraissemblance $L(\theta)$ donne la probabilité d'obtenir l'échantillon observé $\{x_1, \dots, X_n\}$ 
    étant donné un paramètre $\theta$. Elle est définie par: 
        \[ \boxed{ L(\theta) = \myP(X_1 = x_1, X_2 = x_2, \dots, X_n = x_n, \theta) = \prod_{i=1}^{n} f_X(x_i, \theta) } \] 
...
\textbf{L'estimateur du maximum de vraissemblance} $\hat{\theta}$ est la valeur des paramètres $\theta$ qui maximise 
...
\begin{definition}[Information de Fisher]
    ...
    Plus formellement: 
        \[ \boxed{ I(\theta) := - \E \left( \frac{\partial^2}{\partial \theta^2} l(\theta) \right) } \] 
