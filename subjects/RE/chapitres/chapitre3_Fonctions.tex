...
\begin{definition}[Fonction bijective]
    Une fonction $f : E \longrightarrow F$ est dite \emph{bijective} si elle est à la fois injective et surjective. Autrement dit, chaque élément de $F$ possède un antécédent unique dans $E$.
\end{definition}

\begin{example}[Fonction bijective]
    La fonction $f : \R \longrightarrow \R$ définie par $f(x) = x + 1$ est bijective :
    \[
...
\begin{prop}[Caractérisation]
    Une fonction $f : E \longrightarrow F$ est bijective si et seulement si elle est injective et surjective.
\end{prop}

\begin{remark}
    Les fonctions bijectives sont importantes car elles admettent une \emph{fonction inverse} $f^{-1} : F \longrightarrow E$ telle que :
    \[
